        ...
        environment = {
          ...
          ALLOWED_IPS_CMS = base64encode(
             jsonencode([
              ...
              "location = /update.php { allow 149.142.0.0/16; allow 149.143.0.0/16; allow 149.144.0.0/16; allow 159.142.0.0/16; allow 192.168.50.0/24; ${join(" ", [for ip in jsondecode(base64decode(globals.env.apps.waf.environment.ALLOWED_IPS_CMS)) : ip if !contains(["allow 149.142.0.0/16", "allow 149.143.0.0/16", "allow 149.144.0.0/16", "allow 159.142.0.0/16", "allow 192.168.50.0/24", "location = /update.php"], ip)])} proxy_pass ${local.cms_fqdn}; }"
            ])
          )
