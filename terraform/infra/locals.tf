        ...
        environment = {
          ...
          ALLOWED_IPS_CMS = base64encode(
             jsonencode([
              ...
              "location = /update.php { ${join(" ", [for ip in jsondecode(base64decode(local.env.apps.waf.environment.ALLOWED_IPS_CMS)) : ip if !contains(["location = /update.php"], ip)])} proxy_pass ${local.cms_fqdn}; }",
            ])
          )
